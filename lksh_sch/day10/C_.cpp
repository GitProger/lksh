...
inline void solve() {
    int n; cin >> n;
    vector<IndexedPoint> points(n);
    for (int i = 0; i < n; i++) {
        cin >> points[i].value;
        points[i].index = i + 1;
    }

    vector<IndexedPoint> convex;
    IndexedPoint p = *min_element(all(points));

    for (auto &e : convex) e.value -= p.value;
...
//        return atan2(a.value.y, a.value.x) < atan2(b.value.y, b.value.x);
//        a.value -= p.value; b.value -= p.value;
//        int c = a.value.cross(b.value);
//        return c > 0 || (c == 0 && a.value.len2() < b.value.len2());        
    });
    ...
    for (auto e : points) {
        int sz = len(convex);
        while (sz > 1 && !ok(convex[sz - 2].value, convex[sz - 1].value, e.value)) {
            convex.pop_back();
            sz--;
        }
        convex.push_back(e);
    ...
    for (auto p : convex) {
        convex_val.push_back(p.value);
        convex_ind.push_back(p.index);
    }
